        ...
        private void fillTheDataGrid()
        {
            ...
            try
            {
                radGridView1.Invoke(new MethodInvoker(delegate ()
                {
                    ...
                    if(dateS == null)
                        _bindingSource = new BindingSource { DataSource = commandServer.DataGridSet(@"select * from ListSurvey() order by [Дата] desc").Tables[0] };
                    else
                        _bindingSource = new BindingSource { DataSource = commandServer.DataGridSet(@"select * from ListSurvey()
