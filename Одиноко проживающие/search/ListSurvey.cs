...
{
    public partial class ListSurvey : RadForm
    {
        private BindingSource _bindingSource;
        private DateTime ?dateS;
        private DateTime ?dateE;

        public ListSurvey(DateTime start, DateTime end)
        {
            InitializeComponent();
            dateS = start;
            dateE = end;
        ...
        private void fillTheDataGrid()
        {
            ...
            try
            {
                radGridView1.Invoke(new MethodInvoker(delegate ()
                {
                    ...
                    if(dateS == null)
                        _bindingSource = new BindingSource { DataSource = commandServer.GetDataGridSet(@"select * from ListSurvey() order by [Дата] desc").Tables[0] };
                    else
                        _bindingSource = new BindingSource { DataSource = commandServer.GetDataGridSet(@"select * from ListSurvey()
        ...
        private void ListSurvey_FormClosing(object sender, FormClosingEventArgs e)
        {
            Dispose();
            GC.Collect();
